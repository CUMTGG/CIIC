...
def train(opt):
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'bert', 'm2transformer', 'CIIC',
    ...
    try:
        while True:
            ...
            tmp = [data['fc_feats'], data['att_feats'], data['iod_feats'], data['labels'], data['masks'], data['att_masks'], data['iod_masks']]
            tmp = [_ if _ is None else _.cuda() for _ in tmp]
            fc_feats, att_feats, iod_feats, labels, masks, att_masks, iod_masks = tmp

            optimizer.zero_grad()

            model_out = dp_lw_model(fc_feats, att_feats, iod_feats, labels, masks, att_masks, iod_masks, data['gts'],
