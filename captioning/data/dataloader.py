...
class Dataset(data.Dataset):
    ...
    # def get_glove_embedding_dict(self):
    #     return np.load(self.glove_embedding_dict)
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            # fetch image
            tmp_fc, tmp_att, tmp_iod, tmp_seq, \
        ...
        fc_batch, att_batch, iod_batch, label_batch, gts, infos = \
            zip(*sorted(zip(fc_batch, att_batch, iod_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        data = {}
        data['fc_feats'] = np.stack(fc_batch)
        # merge att_feats
        max_att_len = max([_.shape[0] for _ in att_batch])
        max_vc_len = max([_.shape[0] for _ in iod_batch])
        data['att_feats'] = np.zeros([len(att_batch), max_att_len, att_batch[0].shape[1]], dtype='float32')
        for i in range(len(att_batch)):
            data['att_feats'][i, :att_batch[i].shape[0]] = att_batch[i]
        data['iod_feats'] = np.zeros([len(iod_batch), max_vc_len, iod_batch[0].shape[1]], dtype='float32')
        for i in range(len(iod_batch)):
            data['iod_feats'][i, :iod_batch[i].shape[0]] = iod_batch[i]
        data['att_masks'] = np.zeros(data['att_feats'].shape[:2], dtype='float32')
        for i in range(len(att_batch)):
            data['att_masks'][i, :att_batch[i].shape[0]] = 1
        data['iod_masks'] = np.zeros(data['iod_feats'].shape[:2], dtype='float32')
        for i in range(len(iod_batch)):
            data['iod_masks'][i, :iod_batch[i].shape[0]] = 1
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            iod_feat = self.iod_loader.get(str(self.info['images'][ix]['id']))
            # assert att_feat.shape[0]==att_feat_vc.shape[0]
            # att_feat=np.hstack((att_feat,att_feat_vc))
            # Reshape to K x C
            att_feat = att_feat.reshape(-1, att_feat.shape[-1])
            iod_feat = iod_feat.reshape(-1, iod_feat.shape[-1])
