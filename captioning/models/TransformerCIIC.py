...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, iod_feats, tgt, src_mask, iod_masks, tgt_mask):
        "Take in and process masked src and target sequences."
        return self.decode(self.encode(src, iod_feats, src_mask, iod_masks), src_mask,
                           tgt, tgt_mask)

    def encode(self, src, iod_feats, src_mask, iod_masks):

        return self.encoder(self.src_embed(src), iod_feats, src_mask, iod_masks)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, y, mask, iod_mask):
        ...
        y = self.norm(y)
        x = self.sublayer[0](x, lambda x: self.src_attn(x, y, y, iod_mask) + self.self_attn(x, x, x, mask))
...
class Causual_intervention (nn.Module):
    def __init__(self, opt):
        ...
        self.z1 = nn.Parameter(torch.from_numpy(np.load(opt.lin_dict)).cuda().to(torch.float32).requires_grad_())
        self.z2 = nn.Parameter(torch.from_numpy(np.load(opt.visual_dict)).cuda().to(torch.float32).requires_grad_())
...
class CIIC(AttModel):
    ...
    def __init__(self, opt):
        ...
        self.att_embed_iod = nn.Sequential(*(
                ((nn.BatchNorm1d(self.att_feat_size),) if self.use_bn else ()) +
                (nn.Linear(self.iod_feat_size, self.d_model),
                 nn.ReLU(),
                 nn.Dropout(self.drop_prob_lm)) +
                ((nn.BatchNorm1d(self.d_model),) if self.use_bn == 2 else ())))
        delattr(self, 'embed')
    ...
    def _prepare_feature(self, fc_feats, att_feats, iod_feats, att_masks, iod_masks):

        att_feats, iod_feats, seq, att_masks, iod_masks, seq_mask = self._prepare_feature_forward(att_feats, iod_feats, att_masks, iod_masks)
        memory = self.model.encode(att_feats, iod_feats, att_masks, iod_masks)
        ...
        if seq is not None:
            ...
            if seq_per_img > 1:
                att_feats, att_masks, iod_feats, iod_masks = utils.repeat_tensors(seq_per_img,
                                                            [att_feats, att_masks, iod_feats, iod_masks]
                                                            )
        else:
            seq_mask = None
    ...
    def _forward(self, fc_feats, att_feats, iod_feats, seq, att_masks=None, iod_masks=None):
        if seq.ndim == 3:  # B * seq_per_img * seq_len
            seq = seq.reshape(-1, seq.shape[2])
        att_feats, iod_feats, seq, att_masks, iod_masks, seq_mask = self._prepare_feature_forward(att_feats, iod_feats, att_masks, iod_masks, seq)

        out = self.model(att_feats, iod_feats, seq, att_masks, iod_masks, seq_mask)
